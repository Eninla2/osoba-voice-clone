...
print(f"[OSOBA] Edge TTS Voice Studio READY")
...
async def tts_chunk(text, voice):
...
async def generate_audio(text, voice):
    ...
    print(f"[OSOBA] {len(text.split())} words → {len(chunks)} chunks | voice: {voice}")
    parts = []
    for i, chunk in enumerate(chunks):
        print(f"[OSOBA] Chunk {i+1}/{len(chunks)}")
        parts.append(await tts_chunk(chunk, voice))
    return b"".join(parts)

@app.get("/", response_class=HTMLResponse)
def root():
    voice_list = "".join(f"<li>{k} — {v}</li>" for k, v in VOICES.items())
    return (
...
async def generate(request: Request):
    ...
    if voice not in VOICES: voice = DEFAULT_VOICE
...
async def generate_file(file: UploadFile = File(...), key: str = Form(default=""), voice: str = Form(default="")):
    ...
    if not voice or voice not in VOICES: voice = DEFAULT_VOICE
    audio = await generate_audio(text, voice)
    return JSONResponse({
        "success": True,
        "audio":   base64.b64encode(audio).decode(),
