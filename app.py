...
async def _load_all_voices():
    ...
    try:
        ...
        for v in ms_voices:
            vid    = v["ShortName"]          # e.g. en-US-GuyNeural
            locale = v.get("Locale", "")     # e.g. en-US
            gender = v.get("Gender", "")     # Male / Female
            # Build a clean display label from locale + gender + name
            lang_part  = _locale_label(locale)
            gender_part = gender if gender in ("Male", "Female") else ""
            # Extract the "Guy" part from "en-US-GuyNeural"
            name_part  = vid.replace(locale + "-", "").replace("Neural", "").strip()
            label = f"{lang_part} {gender_part} — {name_part}"

            voices_dict[vid] = label
            meta_dict[vid]   = v

            # Store supported styles
            raw_styles = v.get("VoiceTag", {}).get("VoicePersonalities", [])
            if not raw_styles:
                raw_styles = v.get("StyleList", [])
            if raw_styles:
                styles_dict[vid] = [s.lower() for s in raw_styles if s]

        VOICES       = voices_dict
        VOICE_META   = meta_dict
        VOICE_STYLES = styles_dict

        english_count = sum(1 for vid in VOICES if vid.startswith("en-"))
        print(f"[OVS v{VERSION}] Loaded {len(VOICES)} total voices "
              f"({english_count} English) from Microsoft")
    except Exception as e:
        # Fallback: minimal English list so server still works
        print(f"[OVS v{VERSION}] Voice fetch failed ({e}) — using minimal fallback")
        VOICES = {
...
async def _tts_chunk(text: str, voice: str, rate: str,
    ...
    """Stream audio from Microsoft. No temp files."""
    last_error = None
    for attempt in range(1, retries + 1):
        try:
            print(f"[OVS] attempt {attempt}/{retries}  voice={voice}  "
                  f"style={style or 'none'}  chars={len(text)}")
            ssml         = _build_ssml(text, voice, rate, style)
            communicate  = edge_tts.Communicate(ssml, voice)
            audio_chunks = []

            # Wrap stream in a 30s timeout — some voices hang indefinitely
            async def _stream():
            ...
            last_error = f"Voice '{voice}' timed out after 30s"
            print(f"[OVS] timeout on attempt {attempt} — voice may be unavailable")
            # On timeout, fall back to default voice
            if attempt == retries:
                print(f"[OVS] falling back to default voice")
                voice = "en-US-GuyNeural"
                style = ""
        except Exception as e:
            last_error = str(e)
            print(f"[OVS] attempt {attempt} error: {e}")
            # If style caused an error, retry without style
            if style and attempt == 1:
                print(f"[OVS] retrying without style")
                style = ""
        if attempt < retries:
